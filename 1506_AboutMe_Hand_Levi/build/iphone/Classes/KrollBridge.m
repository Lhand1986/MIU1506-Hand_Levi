...
NSString * k1506_AboutMe_Hand_Levi$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1506_AboutMe_Hand_LeviObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1506_aboutme_hand_levi);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k1506_AboutMe_Hand_Levi global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1506_AboutMe_Hand_Levi" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1506_aboutme_hand_levi = [[k1506_AboutMe_Hand_LeviObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1506_aboutme_hand_levi];
	
	NSString *_k1506_aboutme_hand_leviNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1506_aboutme_hand_leviNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1506_aboutme_hand_levi valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1506_AboutMe_Hand_Levi$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1506_aboutme_hand_levi.kroll" 
