...
//Rare exceptions to indicate a bug in the _k1506_aboutme_hand_levi code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_k1506_aboutme_hand_levi = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_k1506_aboutme_hand_levi]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.k1506_aboutme_hand_levi.module"
